...
devdosvid.blog/            # this repo (flipped private at cutover)
  ...
  slides/                  # git subtree — Vite/reveal.js decks
  keepup/                  # git subtree — Python/uv digest
...
2. **Consolidate into this repo** — root tooling + `keepup` subtree; the
   `slides` subtree is timed with the private flip (see constraints).
...
5. **Cutover + decommission** — domain → Worker, flip private, retire old infra.
...
- **The private slides content can't enter a public repo.** So the `slides`
  subtree waits until the repo is private — or moves in earlier only if the
  GitHub plan lets a private repo keep Pages alive until cutover.
- **keepup's LLM swap (Phase 1) lands before its subtree**, so the merged copy
  already runs on Workers AI.
